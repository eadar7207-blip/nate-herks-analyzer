...
INPUT=$(cat)
SOURCE=$(printf '%s' "$INPUT" | python3 -c "import sys,json; print(json.load(sys.stdin).get('source',''))" 2>/dev/null || echo "")
...
LAST_UPDATE=$(git -C "${CLAUDE_PROJECT_DIR:-$(pwd)}" log -1 --format="%ct" -- latest_analysis.md 2>/dev/null)
